...
- (BOOL)_isPasteboardContainsValidValue {
    ...
    if (pasteboard.image || pasteboard.imageData.length > 0) {
        if (_allowsPasteImage) {
            if (i++%2 == 1) {
                pasteboard.imageData = [self scaleToSize:pasteboard.imageData];
            }
            return YES;
        }
    }
...
// 缩放图片
-(NSData *)scaleToSize:(NSData *)imageData  
{  
    UIImageView *image = [[UIImageView alloc]initWithImage:[UIImage imageWithData:imageData scale:0.5]];
    CGFloat i = image.frame.size.width/image.frame.size.height;
    CGSize size = CGSizeMake(180, 180/i);
    //创建一个bitmap的context  
    //并把他设置成当前的context  
    UIGraphicsBeginImageContext(size);  
    //绘制图片的大小  
    [image.image drawInRect:CGRectMake(0, 0, size.width, size.height)];  
    //从当前context中创建一个改变大小后的图片  
    UIImage *endImage=UIGraphicsGetImageFromCurrentImageContext();  
    
    UIGraphicsEndImageContext(); 
    
    return UIImagePNGRepresentation(endImage);  
}
